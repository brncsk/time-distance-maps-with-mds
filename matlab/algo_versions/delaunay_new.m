function [x, g] = compute_optimal_configuration(P, D)
    ...
    for i = 1:N
        for j = 1:N
            ...
            ORIGBEARING(i, j) = atan2(XLon(i) - XLon(j), XLat(i) - XLat(j));
        end
    end
    ...
    while true
        ...
        for k = 1:N
            ...
            for l = 1:N
                ...
                ANGLE_WEIGHT(k,:) = ANGLE_WEIGHT(k,:) + ...
                    [XLatNew(k) XLonNew(k)] - [XLatNew(l) XLonNew(l)] - ...
                    (D(k, l) * [cos(ORIGBEARING(k, l)) sin(ORIGBEARING(k, l))]);
            end
        end
